...
(define record:<record> (cast struct:<record> Record-TypeTop))
...
(define (record-type-fields rtd)
  ...
  (for/list ([i : Natural (in-range init-field-cnt)])
    ...
    (define (get [rt : <record>])
      (((cast accessor-proc (→ <record> Integer (→ Any)))
        rt i)))
    (define set
      (with-handlers ([exn:fail:contract? (λ (_) #f)])
        (make-struct-field-mutator mutator-proc i)
        (λ ([rt : <record>] v)
          (((cast accessor-proc (→ <record> Integer (→ Any Void)))
            rt i)
           v))))
    (list name get set)))
...
(define (make-record rkd field-vector)
  (define constructor-proc (cast (record-type-constructor rkd) (→ Any * <record>)))
